...
kubectl create secret tls s3-tls --cert=gitlab-cert.crt --key=ca.key -n gitlab 2>/dev/null || true
...
# Deploy external MinIO compatible with ARM64
...
# Create the TLS secret for MinIO Ingress (API and Console)
kubectl create secret tls s3-tls --cert=gitlab-cert.crt --key=ca.key -n minio 2>/dev/null || true
...
# Apply Ingress to expose MinIO (API and web console)
kubectl apply -f "$(dirname "$0")/s3-ingress.yaml"
...
sudo sed -i '' '/s3\.local/d' /etc/hosts >/dev/null 2>&1
sudo sed -i '' '/s3-ui\.local/d' /etc/hosts >/dev/null 2>&1
...
echo ""
echo "MinIO Console (UI) : https://s3-ui.local"
echo "  Username: admin"
echo "  Password: admin123"
echo ""
echo "Kubernetes Dashboard: https://dashboard.local/#/login"
echo ""
echo "Kubernetes Dashboard Token: $DASHBOARD_TOKEN"
echo ""
